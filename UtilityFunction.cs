...
namespace MyDatapreMenu
{
    ...
    class UtilityFunction
    {
        ...
        //        IDataset pDS = pRasBandCol.SaveAs(sOutName, pRWS, "GRID");//"IMAGINE Image"
        //        //pDS.Rename(sOutName);
        //        ///*************生成两个栅格数据集(?)
        ...
        //查找栅格数据层
        public static IRasterLayer FindRasterLayer(string sLayerName, AxMapControl ppAxMapControl)
        {
            IRasterLayer pRasterLyr;
            for (int i = 0; i <= ppAxMapControl.ActiveView.FocusMap.LayerCount - 1; i++)
            {
                if (ppAxMapControl.ActiveView.FocusMap.get_Layer(i) is IRasterLayer)
                {
                    pRasterLyr = ppAxMapControl.ActiveView.FocusMap.get_Layer(i) as IRasterLayer;
        ...
        //////////////上朔追踪分析////////
        public static IFeatureLayer FindFeatLayer(string sLayerName, AxMapControl ppAxMapControl)
        {
            IFeatureLayer pFeatLayer;
            for (int i = 0; i <= ppAxMapControl.ActiveView.FocusMap.LayerCount - 1; i++)
            {
                if (ppAxMapControl.ActiveView.FocusMap.get_Layer(i) is IFeatureLayer)
                {
                    pFeatLayer = ppAxMapControl.ActiveView.FocusMap.get_Layer(i) as IFeatureLayer;
        ...
        public static void SetGraphicsLayer(AxMapControl ppAxMapControl, string strFeatLayerName, string strGraphicsLayerName)
        {
            try
            {
                ...
                {
                    pGraphicsLayer = ppAxMapControl.ActiveView.FocusMap.BasicGraphicsLayer;
                }
                else
                {
                    pComGraphicsLayer = ppAxMapControl.ActiveView.FocusMap.BasicGraphicsLayer as ICompositeGraphicsLayer;
                    ...
                    {
                        pFeatLayer = UtilityFunction.FindFeatLayer(strFeatLayerName, ppAxMapControl);
                        pComGraphicsLayer = ppAxMapControl.ActiveView.FocusMap.BasicGraphicsLayer as ICompositeGraphicsLayer;
                ...
                pGraphicsLayer.Activate(ppAxMapControl.ActiveView.ScreenDisplay);
                ILayer pLayer = pGraphicsLayer as ILayer;
                ppAxMapControl.ActiveView.FocusMap.ActiveGraphicsLayer = pLayer;
            }
            catch (Exception eX)
            {
                MessageBox.Show(eX.Message);
        ...
        public static esriFieldType ParseType2ESRI(string type)
        {
            switch (type)
            {
                case "System.String":
